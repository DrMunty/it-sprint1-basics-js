...
test('01_scope-1: can be used in place of `var`', () => {
  let bandName = 'Queen' // Declara bandName utilitzant 'let'
  let isBestBand = true // Declara isBestBand utilitzant 'let'
...
test('01_scope-7: means that we can declare constant with the same name in block statement', () => {
   const d = 5 // Declara una 'd' utilitzant 'const', assignant-li el valor 5
  if (d !== 5)// BLOC DE CODI
  {
    const d = 10 // Declara una 'd' utilitzant 'const', assignant-li el valor 10
    expect(d).toBe(10)
  }
  else {
  expect(d).toBe(5)
